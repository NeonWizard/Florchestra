...
class Frive:
	...
	def armTick_slide(self):
		...
		pi.write(self.stepPin, self.curState[1]) # TOGGLE the step pin
...
class FriveManager:
	...
	waves = []
	for octave in floppyDelays:
		tmp = []
		for frequency in octave:
			wave = []
			#                           ON   OFF  DELAY
			...
			tmp.append(pi.wave_create()) # create and save id
		waves.append(tmp)
